...
- **知识实验室 (Knowledge Lab) 深度增强**:
  - **交互入口迁移**: 将实验室入口从左侧侧边栏移动至右侧主内容区（EmptyState）右上角，并与“图谱”按钮并列，强化全局功能区的逻辑聚合。
  - **动态角标提示**: 实验室入口新增红点角标，实时显示篮子 (Basket) 中待处理碎片的数量。
  - **合成配方库扩展**: 新增 10 种精选合成预设（包含智能整合、思维框架、关系发现等），并引入图标映射系统。
  - **组件架构重构**: 对 `LabView.jsx` 进行了深度重构。采用常量配置化驱动预设列表，代码逻辑更加清晰，并优化了 UI 视觉间距与交互反馈。
  - **布局稳定性提升**: 为实验室结果展示区域引入 `min-w-0` 与 `break-words` 强制换行策略，彻底解决长文本、URL 或特殊代码块导致的容器水平溢出/挤压布局问题。
