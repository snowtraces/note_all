...
- **Backend (状态管理与批注增强)**:
  ...
  - **检索与图谱全链路打通**: 确保 `done` 状态的笔记在全局搜索 (HybridSearch)、知识图谱 (KnowledgeGraph) 及灵感碰撞 (Serendipity) 中依然保持活跃可见，实现“处理即存入常驻记忆”。
- **Frontend (交互工作流深度重构)**:
  - **沉浸式批注系统**: 在详情页右侧边栏底部新增 **固定吸底 (Sticky Bottom)** 的批注区域。支持在阅读碎片的随时记录深度思考，并一键完成“保存并标注已读”的闭环操作。
  - **右侧布局像素级微调**: 
