...
## [Unreleased]
- **全自动 Wiki 知识合成体系**: 
  - **后端能力增强**: 完善了 `AutoCreateWiki` 接口，支持无标题输入时的自主概念发现，并实现了 `ProcessNoteForWiki` 异步流水线，在笔记入库时自动提取并更新相关词条。
  - **溯源与关系可视化**: 词条详情页现支持展示完整的“来源谱系 (Source Lineage)”，用户可清晰追溯知识的原子来源。
- **UI/UX 深度打磨与交互重构**:
  - **AI 对话历史入口优化**: 将“对话历史”访问按钮从左侧栏移至首页 AI 主输入框旁，建立了更符合直觉的 AI 协同入口。
  - **侧边栏导航精简**: 移除了侧边栏冗余的对话历史入口，保持功能分区的纯净度。
  - **稳定性修复**: 
    - 修复了 `DocDetail` 等组件中由于图标（如 `Code`）缺失导致的渲染崩溃问题。
    - 优化了首页 `EmptyState` 的 JSX 结构，解决了移动端布局抖动与语法冲突。
    - 约束了 `useDataPoller` 仅在笔记主列表视图生效，防止背景轮询导致其他分类视图（如文件、相册）被非预期刷新。
