...
export default function NavRail({
  ...
  return (
    <div className="w-[72px] flex-shrink-0 flex flex-col items-center bg-[#080808] border-r border-white/5 pt-6 pb-6 z-[60]">
      {/* Logo Area */}
      <div className="mb-10 relative group cursor-pointer">
        ...
        <div className="relative w-11 h-11 rounded-xl bg-gradient-to-br from-primeAccent/20 to-black border border-primeAccent/40 flex items-center justify-center text-primeAccent">
          ...
          <button
            ...
            className={`
              ...
              ${item.active
                ...
                : 'text-silverText/40 hover:bg-white/5 hover:text-white'
              }
            `}
          >
            {item.active && (
            ...
            {getBadgeValue(item.id) !== null && (
              <div className="absolute top-2.5 right-2.5 w-4 h-4 rounded-full bg-primeAccent text-black text-[9px] font-bold border-2 border-[#080808] flex items-center justify-center pointer-events-none">
                {getBadgeValue(item.id)}
              </div>
            )}

            {/* Tooltip */}
            <div className="absolute left-[70px] px-2 py-1.5 rounded-md bg-white/10 backdrop-blur-md border border-white/10 text-[11px] text-white opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-[100]">
              {item.label}
            </div>
          </button>
        ))}
