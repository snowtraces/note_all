...
func DescribeImageVlm(imageBytes []byte, mimeType string) (desc string, title string, summary string, tags string, err error) {
	if len(imageBytes) == 0 {
				...
				"content": []map[string]interface{}{
					{
						...
						"text": "你是一个专业的视觉理解分析助手。请对这张图片进行深度识别与分析：\n" +
							...
							`{"title":"简练标题","desc":"图片完整的文本提取及视觉描述内容","summary":"简要图片概括(不超过50字)","tags":"标签1,标签2,标签3"}`,
					},
					{
						"type": "image_url",
	...
	return extract.Desc, extract.Title, extract.Summary, extract.Tags, nil
}

// ExtractSummaryAndTags 是一套专用于清洗和归纳 OCR 碎片文本（大模型提炼师）的功能
// 会强求大模型按标准 JSON 的格式返回以方便结构化持久落库
func ExtractSummaryAndTags(ocrContent string, customPrompt string) (title string, summary string, tags string, err error) {
	if len(ocrContent) == 0 {
	...
	return extract.Title, extract.Summary, extract.Tags, nil
}
