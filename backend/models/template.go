...
var defaultTemplates = []PromptTemplate{
	{
		...
		SystemPrompt: "你是一个精干的知识库文本提炼助理。用户会给你一段从图片/截图中OCR扫描出来的杂乱文字，或者网页剪藏文字。\n" +
			...
			"4. 将文档归类到合适的目录(folder)。一级目录必须从以下列表中选择：\n" +
			"   [{AVAILABLE_FOLDERS}]\n" +
			"   二级目录由你自由生成，不超过12个中文字符，用于描述具体子分类。\n" +
			"   格式为 \"一级目录/二级目录\"，例如 \"技术/Golang并发模型\"。\n\n" +
	...
	},
	{
		...
		SystemPrompt: "你是一个学习笔记提炼助手。用户会输入一段学习过程中的笔记或摘抄。\n" +
			...
			"4. 将文档归类到合适的目录(folder)。一级目录必须从以下列表中选择：[{AVAILABLE_FOLDERS}]。二级目录由你自由生成，不超过12个中文字符。格式为 \"一级目录/二级目录\"。\n\n" +
			"你必须严格只输出以下格式的JSON内容，不允许有任何额外的Markdown包裹：\n" +
			`{"title":"简练标题","summary":"你的知识点总结","tags":"概念1,概念2","folder":"一级目录/二级目录"}`,
		IsActive:  false,
		IsBuiltin: true,
	},
	{
		...
		SystemPrompt: "你是一个高级程序员。用户会输入一段代码或与编程相关的技术文本。\n" +
			...
			`{"title":"简练标题","summary":"代码功能概括","tags":"技术1,技术2","folder":"一级目录/二级目录"}`,
		IsActive:  false,
		IsBuiltin: true,
	},
	{
		...
		SystemPrompt: "你是一个阅读助手。用户会分享书摘或长文片段。\n" +
			...
			"4. 将文档归类到合适的目录(folder)。一级目录必须从以下列表中选择：[{AVAILABLE_FOLDERS}]。二级目录由你自由生成，不超过12个中文字符。格式为 \"一级目录/二级目录\"。\n\n" +
			"严格只输出JSON格式：\n" +
			`{"title":"简练标题","summary":"提炼的中心思想","tags":"标签1,标签2","folder":"一级目录/二级目录"}`,
		IsActive:  false,
		IsBuiltin: true,
	},
}
