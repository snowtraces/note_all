...
- **精细化分词**：基于 `gse` 分词器进行动词/名词提取，自动剥离“我、的、是”等停用词，将 query 转化为核心实体列表。

### 2. 混合检索引擎 (Hybrid Retrieval)
系统并行开启三个维度的检索通道，确保“任意命中的灵感均不被遗忘”：
- **向量检索 (Vector Search)**：基于本地 Embedding 模型的余弦相似度计算，门槛设为 0.3-0.5，负责捕获语义相关的模糊内容。
- **全文检索 (FTS5)**：基于 SQLite 原生 FTS5 引擎，利用 **BM25 算法** 对 OCR 文本和摘要进行高基数匹配。
- **标签雷达 (Tag Search)**：利用改写后的 Token 列表进行 `IN` 匹配，并给予其 **5 倍** 的基础评分加权，确保明确标记的知识始终排在最前。

### 3. 多维重排机制 (Re-Ranking)
检索结果并非简单叠加，而是通过加权计算最终 `Score`：
> **Score = 向量相似度(40%) + 全文匹配(20%) + 标签命中(10%) + 时效性权重(10%) + 知识关联密度(20%)**

- **图谱扩展 (Graph Expansion)**：自动提取 Top 检索项的关联笔记（显式链接），通过图结构补全回答所需的背景知识。
